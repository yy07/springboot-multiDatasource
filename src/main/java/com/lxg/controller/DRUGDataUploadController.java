...
//        //职工信息表
//        List<Map> staffInfo = viewPatientMapper.getdata(dataSource + ".STAFF_INFO", days);
//        drugUpload(staffInfo, StaffInfo.class, staffInfoMapper);
//        //医护人员信息表
//        List<Map> medicalStaffInfo = viewPatientMapper.getdata(dataSource + ".MEDICAL_STAFF_INFO", days);
//        drugUpload(medicalStaffInfo, MedicalStaffInfo.class, medicalStaffInfoMapper);
//        //收费项目目录表
//        List<Map> payItems = viewPatientMapper.getdata(dataSource + ".PAY_ITEMS", days);
//        drugUpload(payItems, PayItems.class, payItemsMapper);
//        //床位使用情况统计
//        List<Map> diRsgBedsInfo = viewPatientMapper.getdata(dataSource + ".DI_RSG_BEDS_INFO", days);
//        drugUpload(diRsgBedsInfo, DiRsgBedsInfo.class, diRsgBedsInfoMapper);
//        //科室信息表
//        List<Map> deptInfo = viewPatientMapper.getdata(dataSource + ".DEPT_INFO", days);
//        drugUpload(deptInfo, DeptInfo.class, deptInfoMapper);
//        //药品基本信息表
//        List<Map> drugDetailInfo = viewPatientMapper.getdata(dataSource + ".DRUG_DETAIL_INFO", days);
//        drugUpload(drugDetailInfo, DrugDetailInfo.class, drugDetailInfoMapper);
//        //药品供应商信息表
//        List<Map> drugSupplierInfo = viewPatientMapper.getdata(dataSource + ".DRUG_SUPPLIER_INFO", days);
//        drugUpload(drugSupplierInfo, DrugSupplierInfo.class, drugSupplierInfoMapper);
//        //药品出入库
//        List<Map> diRsgDrugalterInfo = viewPatientMapper.getdata(dataSource + ".DI_RSG_DRUGALTER_INFO", days);
//        drugUpload(diRsgDrugalterInfo, DiRsgDrugalterInfo.class, diRsgDrugalterInfoMapper);
//        //药品库存信息表
//        List<Map> drugInventoryInfo = viewPatientMapper.getdata(dataSource + ".DRUG_INVENTORY_INFO", days);
//        drugUpload(drugInventoryInfo, DrugInventoryInfo.class, drugInventoryInfoMapper);
//        //药品采购信息明细
//        List<Map> drugPurDetail = viewPatientMapper.getdata(dataSource + ".DRUG_PUR_DETAIL", days);
//        drugUpload(drugPurDetail, DrugPurDetail.class, drugPurDetailMapper);
//        //药品调价历史
//        List<Map> drugAdjHis = viewPatientMapper.getdata(dataSource + ".DRUG_ADJ_HIS", days);
//        drugUpload(drugAdjHis, DrugAdjHis.class, drugAdjHisMapper);
//        //患者基本信息
//        List<Map> patient = viewPatientMapper.getdata(dataSource + ".PATIENT", days);
//        drugUpload(patient, Patient.class, patientMapper);
//        //门(急)诊挂号表
//        List<Map> registrationRecord = viewPatientMapper.getdata(dataSource + ".REGISTRATION_RECORD", days);
//        drugUpload(registrationRecord, RegistrationRecord.class, registrationRecordMapper);
//        //门(急)诊病历
//        List<Map> outpatientEmergencyRecord = viewPatientMapper.getdata(dataSource + ".OUTPATIENT_EMERGENCY_RECORD", days);
//        drugUpload(outpatientEmergencyRecord, OutpatientEmergencyRecord.class, outpatientEmergencyRecordMapper);
//        //西药处方
//        List<Map> westernPrescription = viewPatientMapper.getdata(dataSource + ".WESTERN_PRESCRIPTION", days);
//        drugUpload(westernPrescription, WesternPrescription.class, westernPrescriptionMapper);
//        //门(急)诊结算记录
//        List<Map> outpatientEmergSettlement = viewPatientMapper.getdata(dataSource + ".OUTPATIENT_EMERG_SETTLEMENT", days);
//        drugUpload(outpatientEmergSettlement, OutpatientEmergSettlement.class, outpatientEmergSettlementMapper);
//        //门(急)诊发药记录
//        List<Map> outpatientDispenseRecord = viewPatientMapper.getdata(dataSource + ".OUTPATIENT_DISPENSE_RECORD", days);
//        drugUpload(outpatientDispenseRecord, OutpatientDispenseRecord.class, outpatientDispenseRecordMapper);
//        //入院记录
//        List<Map> admissionRecord = viewPatientMapper.getdata(dataSource + ".ADMISSION_RECORD", days);
//        drugUpload(admissionRecord, AdmissionRecord.class, admissionRecordMapper);
//        //入院诊断
//        List<Map> hospitalizedDiagnosis = viewPatientMapper.getdata(dataSource + ".HOSPITALIZED_DIAGNOSIS", days);
//        drugUpload(hospitalizedDiagnosis, HospitalizedDiagnosis.class, hospitalizedDiagnosisMapper);
//        //住院医嘱
//        List<Map> hospitalizedOrder = viewPatientMapper.getdata(dataSource + ".HOSPITALIZED_ORDER", days);
//        drugUpload(hospitalizedOrder, HospitalizedOrder.class, hospitalizedOrderMapper);
//        //住院发药记录
//        List<Map> hospitalizedDispenseDetail = viewPatientMapper.getdata(dataSource + ".HOSPITALIZED_DISPENSE_DETAIL", days);
//        drugUpload(hospitalizedDispenseDetail, HospitalizedDispenseDetail.class, hospitalizedDispenseDetailMapper);
//        //住院费用明细
//        List<Map> hospitalizedExpsetDetail = viewPatientMapper.getdata(dataSource + ".HOSPITALIZED_EXPSET_DETAIL", days);
//        drugUpload(hospitalizedExpsetDetail, HospitalizedExpsetDetail.class, hospitalizedExpsetDetailMapper);
//        //住院结算
//        List<Map> hospitalizedSettlement = viewPatientMapper.getdata(dataSource + ".HOSPITALIZED_SETTLEMENT", days);
//        drugUpload(hospitalizedSettlement, HospitalizedSettlement.class, hospitalizedSettlementMapper);
//        //出院小结
//        List<Map> dischargeAbstract = viewPatientMapper.getdata(dataSource + ".DISCHARGE_ABSTRACT", days);
//        drugUpload(dischargeAbstract, DischargeAbstract.class, dischargeAbstractMapper);
        //住院病案首页
        List<Map> inpatientMedicalRecord = viewPatientMapper.getdata(dataSource + ".INPATIENT_MEDICAL_RECORD", days);
        drugUpload(inpatientMedicalRecord, InpatientMedicalRecord.class, inpatientMedicalRecordMapper);
        //检查记录
        List<Map> inspectionRecord = viewPatientMapper.getdata(dataSource + ".INSPECTION_RECORD", days);
        drugUpload(inspectionRecord, InspectionRecord.class, inspectionRecordMapper);
        //检验记录
        List<Map> surveyRecord = viewPatientMapper.getdata(dataSource + ".SURVEY_RECORD", days);
        drugUpload(surveyRecord, SurveyRecord.class, surveyRecordMapper);
        //入院评估
        List<Map> admissionAssessment = viewPatientMapper.getdata(dataSource + ".ADMISSION_ASSESSMENT", days);
        drugUpload(admissionAssessment, AdmissionAssessment.class, admissionAssessmentMapper);
        //一般护理记录
        List<Map> generalCareRecord = viewPatientMapper.getdata(dataSource + ".GENERAL_CARE_RECORD", days);
        drugUpload(generalCareRecord, GeneralCareRecord.class, generalCareRecordMapper);
        //出入量记录
        List<Map> intakeOutputRecord = viewPatientMapper.getdata(dataSource + ".INTAKE_OUTPUT_RECORD", days);
        drugUpload(intakeOutputRecord, IntakeOutputRecord.class, intakeOutputRecordMapper);
        //出入量记录用药明细
        List<Map> intakeOutputDurgDetail = viewPatientMapper.getdata(dataSource + ".INTAKE_OUTPUT_DURG_DETAIL", days);
        drugUpload(intakeOutputDurgDetail, IntakeOutputDurgDetail.class, intakeOutputDurgDetailMapper);
        //生命体征测量记录
        List<Map> monitorVitalSignsRecord = viewPatientMapper.getdata(dataSource + ".MONITOR_VITAL_SIGNS_RECORD", days);
        drugUpload(monitorVitalSignsRecord, MonitorVitalSignsRecord.class, monitorVitalSignsRecordMapper);
        //生命体征测量明细记录
        List<Map> monitorVitalSignsDetail = viewPatientMapper.getdata(dataSource + ".MONITOR_VITAL_SIGNS_DETAIL", days);
        drugUpload(monitorVitalSignsDetail, MonitorVitalSignsDetail.class, monitorVitalSignsDetailMapper);
        //24H内入出院记录
        List<Map> h24AdmissionDischargeRecord = viewPatientMapper.getdata(dataSource + ".H24_ADMISSION_DISCHARGE_RECORD", days);
        drugUpload(h24AdmissionDischargeRecord, H24AdmissionDischargeRecord.class, h24AdmissionDischargeRecordMapper);
        //住院病程记录-首次病程记录
        List<Map> firstCourseRecord = viewPatientMapper.getdata(dataSource + ".FIRST_COURSE_RECORD", days);
        drugUpload(firstCourseRecord, FirstCourseRecord.class, firstCourseRecordMapper);
        //住院病程记录-日常病程记录
        List<Map> dailyCourseRecord = viewPatientMapper.getdata(dataSource + ".DAILY_COURSE_RECORD", days);
        drugUpload(dailyCourseRecord, DailyCourseRecord.class, dailyCourseRecordMapper);
        //住院病程记录-上级医师查房记录
        List<Map> superiorDoctorWardround = viewPatientMapper.getdata(dataSource + ".superior_doctor_wardround", days);
        drugUpload(superiorDoctorWardround, SuperiorDoctorWardround.class, superiorDoctorWardroundMapper);
        //疑难病例讨论记录
        List<Map> intractableDiscussrecord = viewPatientMapper.getdata(dataSource + ".intractable_discussrecord", days);
        drugUpload(intractableDiscussrecord, IntractableDiscussrecord.class, intractableDiscussrecordMapper);
        //中药处方
        List<Map> chinesePrescription = viewPatientMapper.getdata(dataSource + ".CHINESE_PRESCRIPTION", days);
        drugUpload(chinesePrescription, ChinesePrescription.class, chinesePrescriptionMapper);
        //中医病案首页
        List<Map> inpatientTcmRecord = viewPatientMapper.getdata(dataSource + ".INPATIENT_TCM_RECORD", days);
        drugUpload(inpatientTcmRecord, InpatientTcmRecord.class, inpatientTcmRecordMapper);
        //中医诊疗记录
//        List<Map> chineseDiagRecord = viewPatientMapper.getdata(dataSource+".CHINESE_DIAG_RECORD", days);
//        drugUpload(chineseDiagRecord,chineseDiagRecord.class,chineseDiagRecord);
    ...
    public void drugUpload(List<Map> maps, Class Class1, BaseMapper baseMapper) {
        for (Map map : maps) {
            try {
                Object object = JSONObject.parseObject(JSONObject.toJSONString(map), Class1);
                int insert = 0;
//                insert = baseMapper.insert(object);
//                if (insert == 0) {
//                    System.out.println("插入失败：" + JSONObject.toJSONString(object));
//                } else {
